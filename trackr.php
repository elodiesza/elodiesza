<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8" />
    <link href="styles.css" rel="stylesheet" type="text/css" />
    <link href="plugins/owl-carousel5-main/owl-carousel/css/bootstrap.min.css" rel="stylesheet" type="text/css" />
    <title>Trackr</title>
</head>
<body>
    <?php 
        $title = "";
        include 'menu.php'; 
    ?>
    <div class="container">
        <h1 class="title">Trackr</h1></br>
        <div style="display:flex; flex-direction: row; flex-wrap: wrap;">
            <h2>Introducing Trackr</h3></br>
            <a href="https://github.com/elodiesza/TrackrTrial/tree/master">
                <img src="img/banner/github.png" style="width: 30px; height:30px; margin-left: 10px">
            </a>
            <a href="https://www.figma.com/file/uSvxxclI2wM8NM5ZHoKiwb/Trackr?type=design&node-id=0%3A1&mode=design&t=iG8x3Ys6uBekEc46-1">
                <img src="img/icons/figma.png" style="width: 20px; height:30px; margin-left: 10px">
            </a>
            <a href="https://cloud.protopie.io/p/429358530ec28fbec86db17b">
                <img src="img/icons/protopie.png" style="width: 30px; height:30px; margin-left: 10px">
            </a>
        </div>
        <div style="display:flex; flex-direction:row;width:100%">
            <img src="img/photos/Trackr-icon.png" style="max-width:100px; max-height:100px;">
            <div style=" padding-left: 20px; display:flex; flex-wrap : wrap">
                <h3>Your Path to Personalized Productivity and Well-Being</h3>
                <p>
                    In a world defined by data and constant demands, the need for a tool that enables individuals 
                    to harness their uniqueness and enhance their lives has never been more critical. Enter Trackr, 
                    a groundbreaking life tracker app designed to empower you to seize control of your time, habits, 
                    and overall well-being, paving the way for a more productive, fulfilling life.
                </p></br>
            </div>
        </div>
        </br>
        <h2>How does it work ?</h2></br>
        <h3>What you have to do : </h3>
        <p>
            </br>1. Define the habits (bad or good) you want to track and record your data every day in 2 taps.
            </br>2. Link your habits to your device's existing records (e.g. steps, sleep, etc.) and forget about it.
            </br></br>That's it. Trackr will do the rest.</br>
            </br></br><h3>What Trackr does : </h3>
            </br>1. Provide you with personalized dashboards to overview your progress.
            </br>2. Analyze your data in the background and find correlations between your habits.
            </br>3. Provide you with insights and recommendations without any additional input or request.
        </p></br></br>
        <img src="img/photos/trackr-thumbnail.png" style="max-width:300px; max-height:300px; margin: 20px">
        <details>
            <summary>Value proposition</summary> 
  
            <h3>Unleashing Your Uniqueness in a Data-Driven World</h3>
            <p>Today's reality is flooded with information, yet much of it doesn't directly apply to us. The challenge 
                lies in turning data into actionable insights, steering us toward better life balance, enhanced habits, 
                and a heightened sense of self-discipline. Trackr addresses this gap, offering a platform to not only 
                collect data but to understand and optimize it for your individual journey.
            </p></br>
            <h3>Empowering Personal Growth in a Fast-Paced Landscape</h3>
            <p>In an era defined by technological evolution and rising demands, Trackr emerges as a beacon of innovation, 
                providing a personalized approach to life tracking and analysis. Just as the internet revolutionized 
                collaboration and smartphones connected people across the globe, Trackr aims to revolutionize personal 
                productivity and well-being. It serves as a comprehensive life tracker, habits analyzer, and advisor, 
                catering to the uniqueness of each individual.
            </p></br>
            <h3>Customization as the Key to Transformation</h3>
            <p>Trackr is not a one-size-fits-all solution; it's a dynamic platform that adapts to your specific needs. 
                With the freedom to choose which aspects of your life to monitor, you can effortlessly track everything 
                from sleep patterns and exercise routines to mood and external influences like weather conditions. 
                This customization ensures that Trackr aligns with your goals and aspirations, enabling you to craft 
                a tailored path toward a better version of yourself.
            </p></br>
            <h3>From Data to Meaningful Action: Your Personal Analytics Companion</h3>
            <p>Trackr goes beyond data collection by utilizing advanced algorithms to find correlations and unveil 
                meaningful patterns in your daily life. By transforming these insights into personalized advice, 
                Trackr empowers you to make informed decisions about your habits and routines. It’s like having a 
                personal analytics companion guiding you toward optimal choices that suit your unique circumstances.
            </p></br>
            <h3>A Motivational Catalyst for Success</h3>
            <p>Consistency is the cornerstone of progress, and Trackr recognizes this. By tracking your activities 
                and progress over time, the app becomes a powerful motivator, holding you accountable and committed 
                to achieving your goals. Whether it's cultivating healthy habits, staying active, or embarking on a 
                transformative journey, Trackr rewards your commitment and empowers you to attain your highest potential.
            </p></br>
            <h3>Visualizing Growth: Your Journey Unfolded</h3>
            <p>With Trackr, your journey toward personal growth and well-being becomes tangible and measurable. The app 
                provides comprehensive statistics, allowing you to analyze your progress on a monthly and yearly basis. 
                By comparing different periods, identifying trends, and witnessing your growth firsthand, Trackr enables 
                you to visualize your transformation.
            </p></br>
            <h3>Join the Revolution: Shape a Future of Empowerment</h3>
            <p>Investing in Trackr isn't just about embracing a cutting-edge solution; it's about joining a movement to 
                redefine personal productivity and well-being. In a world where time is precious and demands are ceaseless, 
                Trackr equips you with the tools to thrive. It's time to reclaim control over your life, optimize your 
                uniqueness, and unlock your fullest potential with Trackr.
            </p></br>
            <h3>Don't Wait: Seize Control with Trackr</h3>
            <p>The world is racing against time. Trackr is your ally in the pursuit of meaningful growth and well-being. 
                Embrace the future where personalization meets productivity. Embrace Trackr and embark on a journey to a 
                more enriched, empowered, and exceptional you. Together, let's reshape what's possible and forge a path 
                toward boundless success and fulfillment.
            </p>
        </details>
        </br>
        <iframe max-width="600px" height="auto" src="https://youtube.com/embed/meF-XBE2qZE" frameborder="0" allowfullscreen></iframe>
        </br></br></br>
        <h2>Sample use cases</h3>
        <p>
            </br>1. Most of foreigners moving to South Korea experience noticable hair loss. While doctors blame stress, victims of this phenomenon
            are divided between the country's water quality and air pollution. Trackr could finally find a consensus, or what mostly impacts each person, 
            individually, to prevent further problems.
            </br></br>2. Nelson wants to quit smoking, but his motivation is very irregular. Tracker will not only offer him intrinsic motivation by showing 
            his progress, but also uncover other habits that could influence (or not) his will to return to bad habits.
            </br></br>3. Beata has been suffering from serious headaches during her periods for over 20 years. No doctor was able to find out the cause of her agony.
            Trackr will alow her to track all possible habits that could influence her headaches, and follow-up different habits to check the validity of 
            the correlation and maybe finally overcome this illness she suffered from for decades. Beata is my mom.
        </p></br>
        </br><h2>The story behind it</h3></br>
        <p>
            I have been recording my habits consistently since 2018, first in a notebook, following the bullet journal trend. 
            I realized the power of tracking and the impact it had on my life in 2020 when I moved to Korea with a light luggage,
            leaving it behind. Upon arrival during my quarantine, I decided that it is time to improve my tracking system, learning 
            Swift with the goal of creating a convenient mobile app. As a novice, I went through up and downs, but finally came up
            with a functional version in 2022, and deployed for personal use in 2023. Using it on a daily basis, I am self-testing 
            the functions and optimizing every aspect of it, including UX design that became a new interest of mine.
        </p></br>
        <div style="display:flex; flex:1; justify-content:center; flex-direction: row; padding: 20px; flex-wrap: wrap">
            <div class="gallery-div">
                <div class="thumbnail" onclick="openLightbox('img/photos/bullet1.png')">
                    <img src="img/photos/bullet1.png">
                </div>
                <div class="thumbnail" onclick="openLightbox('img/photos/bullet2.png')">
                    <img src="img/photos/bullet2.png">
                </div>
                <div class="thumbnail" onclick="openLightbox('img/photos/bullet3.png')">
                    <img src="img/photos/bullet3.png">
                </div>
            </div>
        </div>
        </br>
        <h2>And now ?</h3></br>
        <p>
            I am primarily developping Tracker for my own use and mor my vincinity, so I take my time to develop features I consider useful first.
            I have been using paper bullet books for over 6 years and moved to excel 3 years ago to allow for effective calculation. 
            </br>
            <div style="display:flex; justify-content:center; flex-direction: row; padding: 20px; flex-wrap: wrap; width:100%">
                <img src="img/photos/exceltrackr.png" style="max-width:80%; height:auto; object-fit: contain">
            </div>

            </br>I still keep a paper 
            version to keep souvenirs. In fine, I am looking for a way to create an adaptative physical notebook in which I can physically store all data 
            collected on Trackr and customized on the app, to be able to add physical notes and souvenirs such as flight tickets, pictures, cards, etc... 
            Customization and Uniqueness is the essence of Trackr.
        </br>
        </br>
            I am using my beta version of the app on test flight on a daily basis to record my data and it is already more performant and ergonomic than my 
            latest programmed excel file. 
        </br></br> I am currently finishing to develop the algorithm to take into account all types of indicators and include time shifts. In parallel,
            I am working on improving the design, UX and UI.
        </p></br>
        <div style="display:flex; flex:1; justify-content:center; flex-direction: row; padding: 20px; flex-wrap: wrap">
            <div class="gallery-div">
                <div class="thumbnail" onclick="openLightbox('img/photos/ux0.png')">
                    <img src="img/photos/ux0.png">
                </div>
                <div class="thumbnail" onclick="openLightbox('img/photos/ux1.png')">
                    <img src="img/photos/ux1.png">
                </div>
                <div class="thumbnail" onclick="openLightbox('img/photos/ux2.png')">
                    <img src="img/photos/ux2.png">
                </div>
                <div class="thumbnail" onclick="openLightbox('img/photos/ux3.png')">
                    <img src="img/photos/ux3.png">
                </div>
                <div class="thumbnail" onclick="openLightbox('img/photos/ux4.png')">
                    <img src="img/photos/ux4.png">
                </div>
            </div>
        </div>
        </br>
        <h2>The power of Trackr unleashed ?</h3></br>
        <h3>Future functions that could change the game : </h3>
        <p>
        The existing functions of the app already bring a lot of value to the user: But the potential is much bigger than this. Possible ways of expanding the functions are :
        </br></br>- Creating a global database joining all agreeing users’ data in an anonymous way into a common pool to improve recommendation process and decision making based on what works for most people with similar habits.
        </br></br>- Allowing social networking functions such as connecting with people with similar habits/ passions/leisure. This social network would not allow people to share their picture, the main focus will be a person’s activity. This way, the current problem of existing social media : Being unable to judge the potential of a relationship because we are triggered by appearance and faceable discussions rather than actual activities of a person that reveals a lot about someone and the possibility of matching with a person since we do the same things. This will also allow to form groups for activities and share motivational goals, and aspire others to keep up with the group’s progress. 
        </br></br>- A reward function will complement this social media function, making it possible for users to share their accomplishments rewards on the platform or externally on other social medias. Rankings could be done, or competitions/challenges could be organized so people can improve their skills /habits with a tangible goals and give the best of themselves. Challenges, more than competition, can install a positive atmosphere and bring people together to achieve the same goal.
        </br></br>- Reliable marketing tool and tester marketplace : Brands could use the app to find testers for their products by adding the use of their products in their habits and share their results with the world. Tester will be paid a small amount by the brand. If the product is good, the brand will naturally benefit from the good effects proven by the users’ tracks. Since testers are assigned by the app (and not chosen by the brand) the users can be more assured of the reliability of the results not manipulated nay the brand, and be more confident of the good effects of the product if the results are such. It will then help the brand to market the product, and the users to find a product that they will feel confident to buy. Also, it will be interesting to see if the profile of the testers matches the one on the user (for drugs for example, it will be interesting to see if the testers have the same habits as the user who wants to buy the drug, to avoid side effects)
        </p></br>
        <div style="display:flex; flex:1; height: 40px; justify-content:flex-end">
            <img src="img/icons/TS.png" style="object-fit: contain">
            <img src="img/icons/react.png" style="object-fit: contain">
        </div>
        </br>
    </div>
    <?php include 'scroll-up.php'; ?>
    <div class="footer">
        <p style="text-align: center">This is my resume <span style="color:#949B40">v1.0</span></p>
    </div>
        <div id="lightbox" class="lightbox-container" onclick="closeLightbox()">
        <div class="lightbox-content">
            <img id="lightboxImage" class="lightbox-img" src="" alt="Lightbox Image">
            <div id="description" class="lightbox-description"></div>
        </div>
    </div>
    <script>
    function openLightbox(imageSrc, description) {
        const lightbox = document.getElementById('lightbox');
        const lightboxImage = document.getElementById('lightboxImage');
        const descriptionElement = document.getElementById('description');

        lightboxImage.src = imageSrc;
        descriptionElement.innerText = description;
        lightbox.style.display = 'flex';
    }

    function closeLightbox() {
        const lightbox = document.getElementById('lightbox');
        lightbox.style.display = 'none';
    }
    </script>
</body>
</html>